...
function getbooks(hglass)
{
    ...
    $.getJSON('/books/list?pattern='+pattern, function(data){
        ...
        $.each(booklist, function(key, binfo) {
            ...
            '<td>' +
            '<button type="button" class="btn btn-info" onclick="browse(\''+bpath+'\');">Details</button>' +
            '<button type="button" class="btn btn-info" onclick="docmd(\'' +bpath+ '\',\'delete\');">Delete</button>'+
                '</td>'+
            '</tr>';
